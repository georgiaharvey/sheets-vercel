...
function aggregateBGTableSalesBiweekly(rawRows) {
  ...
  for (const row of rawRows) {
    ...
    const dateMatch = rowText.match(monthRegex);
    if (dateMatch) {
      currentDate = new Date(`${dateMatch[1]} ${dateMatch[2]}, ${currentYear}`);
      continue;
    }
    if (currentDate) {
      for (const cell of row) {
        if (cell && typeof cell === 'string' && cell.includes('$')) {
           ...
           if (!isNaN(gross)) {
             const periodStart = getBiweeklyPeriodStart(currentDate);
             const periodKey = periodStart.toISOString();
             biweeklyTotals[periodKey] = (biweeklyTotals[periodKey] || 0) + gross;
           }
        }
      }
    }
